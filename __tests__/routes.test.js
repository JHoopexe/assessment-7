...
describe("POST /auth/register", function() {
  ...
  // TEST BUG #5
  test("should allow a user to register as an admin", async function() {
    const response = await request(app)
      .post("/auth/register")
      .send({
        username: "new_user1",
    ...
    let { username, admin } = jwt.verify(response.body.token, SECRET_KEY);
    expect(username).toBe("new_user1");
  ...
  // TESTS BUG #1
  test("should not allow a user to register with incomplete data", async () => {
    const response = await request(app)
      .post("/auth/register")
      .send({
        username: "abc",
...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #2
  test('should not allow a incorrect username to log in', async () => {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "abc",
  ...
  test('should not allow a incorrect password to log in', async () => {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
